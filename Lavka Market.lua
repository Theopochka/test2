...
local lmUrl = "https://www.mediafire.com/file/x4ypbqvu272swog/Lavka+Market.lua/file?dkey=2ygzsr8f6ea&r=614"
...
recMenu[0] = not recMenu[0]
    ...
    imgui.Begin('Cerrntral rrADD Vip Version', outhmenu, imgui.WindowFlags.AlwaysAutoResize + imgui.WindowFlags.NoTitleBar)
            if imgui.InputText(u8"NickName", NickName, sizeof(NickName)) then
            ini.autologin.nickname = str(NickName)
            cfg_save()
        end
        if imgui.InputText(u8"Password", Password, sizeof(Password)) then
...
if imgui.Button(faicons("LIST") .. u8' Информация', imgui.ImVec2(200, 50)) then tab = 4 end
...
imgui.Separator()
if imgui.Checkbox(u8"Автоeда", autoeat) then
...
cfg_save()
   ...
   if autoeat[0] then
    ...
    if imgui.Button('u8Обновить(возможно зависание игры на 10-15 секунд)') then
            ...
            if(raknetBitStreamReadInt8(bs) == 9) then 
                ...
                if(show_type == 3) then
                    ...
                    if(level_type == 5) then
                        lua_thread.create(function()
                            wait(200)
                            msg("Выбираем место спавна - "..(item_spawn[ini.autologin.spawn_use+1]))                      
